...
public class DatabasePersonLoader implements PersonLoader {
    ...
    public Person[] load() {
        try {
            return proccessPeople(connection.createStatement().
    ...
    private Person[] proccessPeople(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next())
            personList.add(proccessPerson(resultSet));
        
        return personList.toArray(new Person[personList.size()]);
    }

    private Person proccessPerson(ResultSet resultSet) throws SQLException {
